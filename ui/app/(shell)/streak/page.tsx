...
import { formatDateSafe, getTodayString } from '@/lib/datetime'
...
// Strip metadata suffixes like "| duration: 60 | priority: high" from task text
...
export default function StreakPage() {
  ...
  const loadChallenges = async () => {
    try {
      ...
      // Load today's tasks from the from-challenges API (timezone-aware)
      const tz = Intl.DateTimeFormat().resolvedOptions().timeZone
      const today = getTodayString(tz)
      let todayTasksByChallenge: Record<string, any[]> = {}
      try {
        let tasksUrl = addProfileId('/api/todos/from-challenges', profileId)
        tasksUrl += (tasksUrl.includes('?') ? '&' : '?') + `timezone=${encodeURIComponent(tz)}`
        const tasksRes = await fetch(tasksUrl)
        const tasksData = await tasksRes.json()
        const allTasks = tasksData.tasks || []
        // Group today's tasks by challengeId
        for (const task of allTasks) {
          if (task.dueDate === today) {
            if (!todayTasksByChallenge[task.challengeId]) todayTasksByChallenge[task.challengeId] = []
            todayTasksByChallenge[task.challengeId].push({
              completed: task.completed || false,
              text: cleanTaskText(task.title || task.text || '')
            })
          }
        }
      } catch {}

      const challengesWithTasks = challengeList.map((challenge: Challenge) => ({
        ...challenge,
        todaysTasks: todayTasksByChallenge[challenge.id] || [],
        totalDayFiles: challenge.totalDays || 0
      }))
              ...
              >
                ...
                {challenge.targetDate && (
                  <div className="flex items-center gap-2 text-xs text-oa-text-secondary mb-3">
                    ...
                    <span>
                      Due: {formatDateSafe(challenge.targetDate)}
                    </span>
                  </div>
                )}
