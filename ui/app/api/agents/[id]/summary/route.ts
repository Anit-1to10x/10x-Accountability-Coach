  ...
  try {
    ...
    for (const folder of folders) {
      ...
      try {
        ...
        const daysMatch = content.match(/\*\*Days Completed:\*\*\s*(\d+)\/(\d+)/i)

        const challengeAgent = agentMatch ? agentMatch[1].trim() : ''

        // Only include challenges for this agent
        if (challengeAgent === agentId || !challengeAgent) {
          challenges.push({
            id: folder,
            ...
            currentDay: daysMatch ? parseInt(daysMatch[1]) : 0,
            totalDays: daysMatch ? parseInt(daysMatch[2]) : 30,
            progress: progressMatch ? parseInt(progressMatch[1]) : 0,
            streak: {
              current: currentMatch ? parseInt(currentMatch[1]) : 0,
              best: bestMatch ? parseInt(bestMatch[1]) : 0,
            },
          })
        }
      } catch {
