    ...
    try {
      ...
      for (const folder of challengeFolders) {
        ...
        if (stat.isDirectory()) {
          try {
            // Read challenge.md
            const challengeMdPath = path.join(folderPath, 'challenge.md')
            const mdContent = (await fs.readFile(challengeMdPath, 'utf-8')).replace(/\r\n/g, '\n')

            const nameMatch = mdContent.match(/^#\s+(.+)$/m)
            const statusMatch = mdContent.match(/\*\*Status:\*\*\s*(.+)/i)
            const currentStreakMatch = mdContent.match(/\*\*Current:\*\*\s*(\d+)/i)
            const lastCheckinMatch = mdContent.match(/\*\*Last Check-in:\*\*\s*(.+)/i)
            const progressMatch = mdContent.match(/\*\*Overall:\*\*\s*(\d+)%/i)
            ...
            const streak = {
              ...
              lastCheckin: lastCheckinMatch && lastCheckinMatch[1].trim() !== 'None' ? lastCheckinMatch[1].trim() : null,
            }
