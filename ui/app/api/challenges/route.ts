...
function parseChallengeMd(rawContent: string, id: string) {
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Use profile-specific challenges directory if profileId provided
    let challengesDir = PATHS.challenges
    if (profileId) {
      const profileChallengesDir = getProfilePaths(profileId).challenges
      try {
        await fs.access(profileChallengesDir)
        const profileDirs = await fs.readdir(profileChallengesDir)
        if (profileDirs.length > 0) {
          challengesDir = profileChallengesDir
        }
      } catch {
        // Fall back to global challenges
      }
    }
    ...
    for (const dir of dirs) {
      if (dir.isDirectory()) {
        try {
          ...
          // Calculate progress — handle 'dynamic' dates for upcoming challenges
          const rawStartDate = config.start_date || config.startDate || ''
          const rawEndDate = config.target_date || config.end_date || config.targetDate || ''
          const isDynamic = rawStartDate === 'dynamic' || rawEndDate === 'dynamic'
          const isToday = rawStartDate === 'today' || rawEndDate === 'auto'
          const startDate = (isDynamic || isToday) ? new Date() : new Date(rawStartDate || new Date())
          const endDate = isDynamic ? new Date(Date.now() + 30*24*60*60*1000) : isToday ? new Date(Date.now() + (totalDayFiles || 7)*24*60*60*1000) : new Date(rawEndDate || Date.now() + 30*24*60*60*1000)
          const totalDays = (isDynamic || isToday) ? (totalDayFiles || 30) : Math.ceil((endDate.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24))

          // Always calculate progress from actual completed days
          const progress = totalDays > 0 ? Math.min(Math.round((completedDays / totalDays) * 100), 100) : 0
          ...
          challenges.push({
            ...
            startDate: isDynamic ? 'dynamic' : isToday ? 'today' : (config.start_date || config.startDate),
            targetDate: isDynamic ? 'dynamic' : isToday ? 'auto' : (config.target_date || config.end_date || config.targetDate),
